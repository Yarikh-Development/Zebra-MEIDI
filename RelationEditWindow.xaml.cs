...
namespace demo
{
	...
	public partial class RelationEditWindow : Window
	{
		...
		public RelationEditWindow(int line)
        {
			...
			XML_TextBox.Text = data[1];
			ZPL_TextBox.Text = data[2];
			Printer.SetPrinters(Printer_ComboBox);
		}
		
		private void SavaRelationRecordClick(object sender, RoutedEventArgs e)
        {
			if (EE_Number_TextBox.Text == "" || XML_TextBox.Text == "" || ZPL_TextBox.Text == "")
				return;
			string data = EE_Number_TextBox.Text + ';' + XML_TextBox.Text + ';' + ZPL_TextBox.Text + ';' + Printer_ComboBox.Text + ';';
			if (line == -1) //添加记录
